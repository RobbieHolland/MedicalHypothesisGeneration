...
class TrainSparseAutoencoder(pl.LightningModule):
    ...
    def _compute_and_log_metrics(self, sae_out, x, frac_active_list, stage="train"):
        l0 = (sae_out["feature_acts"] > 0).float().sum(-1).mean()
        l2_norm = torch.norm(sae_out["feature_acts"], dim=1).mean()
        l2_norm_in = torch.norm(x, dim=-1)
        l2_norm_out = torch.norm(sae_out["sae_out"], dim=-1)
        l2_norm_ratio = l2_norm_out / (1e-6 + l2_norm_in)

        # Fraction of features active
        act_freq_scores = (sae_out["feature_acts"].abs() > 0).float().sum(0)
        ...
        wandb.log(
            {
                ...
                f"{stage}/metrics/l0": l0.item(),
                f"{stage}/metrics/l2": l2_norm.item(),
                f"{stage}/metrics/l2_ratio": l2_norm_ratio.mean().item(),
                f"{stage}/sparsity/below_1e-5": (feature_sparsity < 1e-5).float().mean().item(),
                f"{stage}/sparsity/below_1e-6": (feature_sparsity < 1e-6).float().mean().item(),
                f"{stage}/sparsity/n_dead_features": (
                    feature_sparsity < self.config.task.dead_feature_threshold
                ).float().mean().item(),
            },
...
def fit_sae(config):
    ...
    trainer = pl.Trainer(
        ...
        val_check_interval=config.task.validation_interval,  # runs validation every 500 steps
        callbacks=[pl.callbacks.ModelCheckpoint(dirpath="checkpoints", save_top_k=1, monitor="train_loss")],
    )
