...
class SparseAutoencoder(pl.LightningModule):
    ...
    def forward(self, x: torch.Tensor, dead_neuron_mask: torch.Tensor | None = None):
        ...
        hidden_pre, feature_acts, sae_out = self.forward_topk_sae(x)

        # x_centred = x - x.mean(dim=0, keepdim=True)
        # mse_loss = (
        #     (sae_out - x.float()).pow(2)
        #     / (x_centred.pow(2).sum(dim=-1, keepdim=True).sqrt())
        # ).mean()

        mse_loss = torch.nn.MSELoss()(sae_out, x)
        ...
        sparsity = torch.norm(feature_acts, p=self.config.task.lp_norm, dim=1).mean()
